...
class ParseVOI(object):
    ...
    def one_dict(self,filepath):
        self.PATH=filepath
        if filepath.endswith('.voi'):
            one_dict = self.BBox_from_position(path=filepath)
        return(one_dict)
    ...
    def BBox_from_position(self,path):
        ...
        for slice in dict.keys():
            ...
            try:
                ...
                print("problem with {}".format(slice))
                tuple = (category,-1,-1,-1,-1)
                dict.update({slice:tuple})
        return(dict)
    ...
    def get_ROI_slice_loc(self,path=None):
        ...
        for i in range(len(slice_num_list)):
            ...
            if i<(len(slice_num_list)-1):
                ...
                slice_num=line_specific.str.split(r"\s")[0][0]
